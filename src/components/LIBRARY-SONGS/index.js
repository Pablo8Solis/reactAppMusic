...
import { removeSong } from "../../Redux/slices/librarySlice"; // Desde el slice

const Library = () => {
    const dispatch = useDispatch();

    // Ahora leemos state.library
                    ...
                    {songs.length > 0 ? (
                        songs.map((song) => (
                            <div key={song.id} className="library-song-item" style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center' }}>
                                ...
                                />
                                <button
                                    style={{ backgroundColor: 'grey', color: 'white', border: 'none', cursor: 'pointer', fontSize: '1.5rem', height: '4rem', borderRadius: '5px' }}
